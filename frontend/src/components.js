          ...
          {activeTab === 'expertise' && (
            <div className="space-y-6">
              <div className="bg-gray-800 rounded-lg p-6">
                <h3 className="text-xl font-semibold text-white mb-4">🎯 Select Your Expertise</h3>
                <p className="text-gray-400 mb-6">
                  Choose your area of expertise to determine your monthly subscription fee and start accepting consultations.
                </p>
                
                {!hasSetExpertise ? (
                  <div className="space-y-4">
                    <h4 className="text-lg font-medium text-white">Available Categories:</h4>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {expertiseCategories.map(category => (
                        <div 
                          key={category.id}
                          onClick={() => {
                            setSelectedExpertise(category.id);
                            setMonthlyFee(category.fee);
                          }}
                        ...
                        >
                          <div className="flex justify-between items-start mb-2">
                            ...
                            <span className={`px-2 py-1 text-xs rounded ${
                              category.demand === 'high' ? 'bg-green-500/20 text-green-400' : 'bg-yellow-500/20 text-yellow-400'
                            }`}>
                              {category.demand === 'high' ? 'High Demand' : 'Medium Demand'}
                            </span>
                          </div>
                          <div className="text-2xl font-bold text-green-400">${category.fee}/month</div>
                          <p className="text-gray-400 text-sm">Professional consulting services</p>
                        </div>
                      ))}
                    </div>
                    ...
                    {selectedExpertise && (
                      <div className="mt-6 p-4 bg-blue-500/10 border border-blue-500 rounded-lg">
                        <h5 className="font-semibold text-white mb-2">Selected: {expertiseCategories.find(c => c.id === selectedExpertise)?.name}</h5>
                        <p className="text-gray-300 mb-4">Monthly Fee: ${monthlyFee}</p>
                        <button
                          onClick={() => {
                            setHasSetExpertise(true);
                            // Redirect to subscription page
                            window.location.href = `/subscription?expertise=${selectedExpertise}&fee=${monthlyFee}`;
                          }}
                  ...
                  <div className="p-4 bg-green-500/10 border border-green-500 rounded-lg">
                    <h5 className="font-semibold text-white mb-2">✅ Expertise Set</h5>
                    <p className="text-gray-300">You've selected: {expertiseCategories.find(c => c.id === selectedExpertise)?.name}</p>
                    <p className="text-gray-300">Monthly Fee: ${monthlyFee}</p>
                    <button
                      onClick={() => setHasSetExpertise(false)}
